...
3. The Byte-Pair Encoding (BPE) tokenizer provides a 1.95x compression ratio on the training corpus, which effectively increases the sequence length and context window of the model without additional compute.
...
10. This configuration achieves a bits-per-byte (bpb) of 1.8856, representing a massive improvement over the baseline (2.3718) while training in 320 seconds on CPU.
